...
class MarketTerminal:
    ...
    def _build_log(self, parent):
        ...
        self.log_text = tk.Text(frame, font=(FONT_MONO, 8),
                                ...
                                state=tk.DISABLED, height=7, wrap=tk.WORD)
    ...
    def _fetch_loop(self, symbol):
        prev_last = None
        tick_count = 0
        while self.running and self.current_symbol == symbol:
            t0 = time.time()
            try:
                ...
                self.quote_history.append(last)
    ...
    def _update_ui(self, bid, ask, last, chg, chg_color,
                   hi, lo, ticks, avg, latency, symbol):
        if self.current_symbol != symbol:
            return
        self.price_var.set(f"{last:,.2f}")
    ...
    def _quick_select(self, sym):
        ...
        self.root.after(800, self.start_stream)
